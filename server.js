...
async function handleApi(req, res, url) {
  ...
  if (req.method === 'POST' && ['/api/auth/forgot-password', '/api/auth/forgot', '/api/auth/request-password-reset', '/api/auth/reset-request'].includes(pathname)) {
    const body = await readBody(req);
    const email = String(body.email || '').trim().toLowerCase();
  ...
  if (req.method === 'POST' && pathname === '/api/auth/reset-password') {
    ...
    if (!token || password.length < 6) return json(res, 400, { error: 'Geçersiz link veya şifre.' });

    const reset = (db.passwordResets || []).find((item) => item.token === token && !item.usedAt);
    if (!reset || new Date(reset.expiresAt).getTime() < Date.now()) return json(res, 400, { error: 'Şifre yenileme linki süresi dolmuş veya geçersiz.' });
